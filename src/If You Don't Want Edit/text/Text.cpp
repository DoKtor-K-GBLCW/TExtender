...
wchar*
CText::Get(const char *key, bool returnNull)
{
	auto res = CNewTextLoader::Get(key);
	if (res)
		return res; // found in fxt files, return

	uint8 result = false;
#if defined (FIX_BUGS) || defined(FIX_BUGS_64)
	wchar *outstr = keyArray.Search(key, data.chars, &result, returnNull);
#else
	wchar *outstr = keyArray.Search(key, &result, returnNull);
#endif

	if (!result && bHasMissionTextOffsets && bIsMissionTextLoaded)
#if defined (FIX_BUGS) || defined(FIX_BUGS_64)
		outstr = mission_keyArray.Search(key, mission_data.chars, &result, returnNull);
#else
		outstr = mission_keyArray.Search(key, &result, returnNull);
#endif
...
char*
CText::GetAsCChar(const char* key, bool returnNull)
{
	auto res = Get(key, returnNull);
	if (!res)
		return returnNull ? NULL : "";

	return Utils::WideCharToConstChar(res);
}
...
wchar*
#if defined (FIX_BUGS) || defined(FIX_BUGS_64)
CKeyArray::Search(const char *key, wchar *data, uint8 *result, bool returnNull)
#else
CKeyArray::Search(const char *key, uint8 *result, bool returnNull)
#endif
{
	CKeyEntry *found;
	char errstr[25];
	...
	*result = false;
	...
	sprintf(errstr, "");
#else
	sprintf(errstr, "%s", key); // was: "%s missing", but its now works like Laravel trans() / __()
#endif // MASTER
	for(i = 0; i < 25; i++)
