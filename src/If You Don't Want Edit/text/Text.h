...
#if defined (FIX_BUGS) || defined(FIX_BUGS_64)
	wchar *Search(const char *key, wchar *data, uint8 *result, bool returnNull = false);
#else
	wchar *Search(const char *key, uint8* result, bool returnNull = false);
#endif
};
...
public:
	...
	void Load(void);
	void Unload(void);
	wchar *Get(const char *key, bool returnNull = false);
	char* GetAsCChar(const char* key, bool returnNull = false);
